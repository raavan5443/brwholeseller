window.onload=function(){const vendorsList=document.getElementById("vendors-list");const loadBtn=document.getElementById("load-btn");const spinnerBox=document.getElementById("spinner-box");const emptyBox=document.getElementById("empty-box");const loadsBox=document.getElementById("loading-box");const vendorsNum=document.getElementById("vendors-number")
let visible=12;const handleGetVendors=(sorted)=>{$.ajax({type:"GET",url:`/vendors-ajax/`,data:{"num_vendors":visible,},success:function(response){const data=response.data;const maxSize=response.max
...
console.log(data)
if(response.vendors_size>0){vendorsNum.innerHTML=`${response.vendors_size}`
data.map(vendor=>{let d=new Date(vendor.date);vendorsList.innerHTML+=`<!--start vendor card-->
...
else{vendorsNum.innerHTML=`0`
vendorsList.innerHTML=``
loadsBox.classList.add("not-visible")}},500)},error:function(error){console.log(error)}})}
handleGetVendors();loadBtn.addEventListener("click",()=>{visible+=12;handleGetVendors(false);})}